...
import { FormUseCase, FormToursUseCase, FormTemplatesUseCase } from "@/server/modules/form/aplication/form.usecase";
...
export const sendFormTemplates = async (req: NextApiRequest, res: NextApiResponse<adapterResponseI>) => {
  try {
    const response = await FormTemplatesUseCase({
      email: req.body.email,
      name: req.body.name,
      phone: req.body.phone,
      ...
      reasonPurchase: req.body.reasonPurchase
    });
